"""
Pesapal API 3.0 FastAPI Backend Implementation

This module provides a FastAPI backend for integrating with Pesapal payment gateway.
...
class PesapalClient:
    """Pesapal API client for making HTTP requests"""
...
app = FastAPI(
    ...
    description="FastAPI backend for Pesapal payment gateway integration",
    version="1.0.0",
    lifespan=lifespan
)
...
async def register_ipn(ipn_data: IPNRegistration):
    """
...
async def submit_order(payment_request: PaymentRequest):
    """
...
async def payment_callback(
    ...
    if OrderTrackingId:
        try:
            ...
            transaction_status = await get_transaction_status(OrderTrackingId)
...
async def payment_ipn(request: Request):
    ...
    try:
        ...
        if order_tracking_id and notification_type == "IPNCHANGE":
            try:
                # Get the actual transaction status
                transaction_status = await get_transaction_status(order_tracking_id)
